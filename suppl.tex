...
attention. Its distribution is 
derived in Lemma~\ref{lem:Full_for_Z}.
...
Guided by the empirical Bayesian approach, we also consider the
following full conditional distributions for the hyperparameters
$\psi$. Observe first that due to independence between the
components of the matrices $A_e$, $B_e$, $A_p$ and $B_p$, the full 
conditional $\pi(\psi|\theta, Z, M, \eta)$ equals the product
\[
...
steps. The implementation of these is described in Section~\ref{sec:MHsteps}.
...
arguments follow closely those in \cite{C01}, but they are of course
adapted to the hierarchical NMF model considered in the main text. 
...
\begin{theorem} Under the conditions in Lemma~\ref{lem:technical}, for
  the statistic defined in \textup{(3)}, 
\[
  %\lim_{R, u \to\infty} 
  \sup_{S \in  \mathcal B} 
     \big| 
...
\[
  \widehat\pi(\theta|M, \hat\eta) \to \pi(\theta|M,
  \eta)\qquad\text{a.e. as } u, R \to \infty.
\]
...
\[
  ...
  \eta)\big|\ \text{d}m(\theta) \to 0 
 \qquad\text{as } u, R\to \infty, 
\]
with $m$ as the Lebesgue measure defined in $(\Theta, \mathcal
B)$. This gives
\begin{align*}
