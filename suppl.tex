...
The plots in Figure~\ref{fig:bcancerDATA} presents the mutation
counts matrix and the estimated matrix. The sample P**** has been left
out for visualisation purposes, because it contains extremely large 
counts.

Plot the exposures as box plots.
...
Section 4.2 in the main text. Figure~\ref{fig:COSMIC} presents the
signatures 1, 2, 3 and 13 obtained from Sanger's catalogue at\\
\indent\url{http://cancer.sanger.ac.uk/cosmic/signatures}\\ 
It it worth observing that this data set is rather challenging because
of the similarity of signatures $S_2$ and $S_4$.
...
data. The actual run shown here was randomly
chosen\footnote{\textcolor{red}{RD: please confirm this}}. Figure~\ref{fig:EMu} presents the signatures estimated by EMu  
in one of the analyses where it correctly estimated the NMF model
rank.\footnote{\textcolor{red}{RD: should by put also 
those by Alexandrov? I know, this implies mentioning all the stuff
related to opportunities?}} The latest version of Emu was obtained by
following the instructions in\\
\indent\url{http://www.sanger.ac.uk/science/tools/emu}\\
which redirects to\\
\indent \url{https://github.com/andrej-fischer/EMu}\\
Figure~\ref{fig:ANZ} presents the signatures obtained in one of the
100 analyses made by using the method in \cite{A}. As mentioned in the 
main text, all these analyses were made by setting the NMF rank to its
correct value. 


The analysis with the framework described in \cite{A} was made by
...
\begin{figure}
 ...
 \end{tabular}
 \caption{The 21 breast cancer data set (left) and the corresponding
% counts matrix $\widehat M$ obtained by the factorisation $ \widehat
% M = \widehat P\widehat E$ for the rank $N=4$.}
...
\caption{Four signatures estimated by \texttt{signeR}.}\label{fig:signeR}
\end{figure}
\end{center}
...
\begin{figure}
  ...
  \caption{Four signatures estimated by EMu}\label{fig:EMu}
\end{figure}
\end{center}
