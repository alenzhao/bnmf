...
\]
The maximization of the second and the fourth of the remaining terms in (\ref{eqn:ell})
...
a Gamma density, which has no closed form solution, see \cite{CW}. 
To circumvent this difficult we used an R package for non-linear optimization, 
nloptr, which is an R interface to NLopt, a free/open-source library for nonlinear 
optimization (Johnson, S.G., http://ab-initio.mit.edu/nlopt). Hyperparameters 
$\hat{a_p}$, $\hat{b_p}$, $\hat{a_e}$ and $\hat{b_e}$ were 
estimated by maximising  the likelihood of the sampled $\psi$ using the Nlopt 
implementation of Tom Rowan's "Subplex" algorithm (Rowan, 1990).
  ...
  \parbox[c][4cm][t]{6.6cm}{
   {\sc Rodrigo D. Drummond\\
    ...
    E-mails: \parbox[t]{2.5cm}{%
      \verb~rdrummond@cipe.accamargo.org.br~\\ 
      \verb~renan.valieris@cipe.accamargo.org.br~}
 }
\\[-2em]
  \parbox[c][4cm][t]{6.6cm}{
