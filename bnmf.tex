...
\abstract{%
\textbf{Motivation:} Tracking the activity of mutational processes
provides an unique opportunity to understand the origins of human 
cancer. These processes have recently been identified with mutational
signatures while analysing high throughput data from cancer
genomes. Mutational signatures are nowadays identified by using
non-negative factorisation (NMF) techniques. An important question in
this context consists in the determination of the actual number of
signatures that best represent the data. Current methods, based on
optimisation techniques, employ ad-hoc heuristics to address this
issue. Moreover, they are strongly dependent on initial conditions 
because the NMF paradigm is highly dimensional and a non-convex
optimisation problem. The extraction of  mutational signatures
from high-throughput data still remains therefore a challenging
question.
\\
\textbf{Results:} 
Here we present a method for the statistical estimation of mutational
signatures based on an empirical Bayesian treatment of the NMF model.
While requiring minimal intervention from part of the user our method
addresses the determination of the number of signatures directly as a
model selection problem. Our modelling introduces two new concepts for
evaluating the mutational profile that are of considerable clinical
interest. The advantages of our approach are shown by the analysis of
real and synthetic data sets.  The later are used to compare our
approach against two alternative methods mostly used in the literature
and with the same NMF parametrisation as the one considered here. Our
approach is robust to initial conditions and more accurate than
competing alternatives. It also estimates the correct number of
signatures even when other methods fail. Results obtained in the
analysis of a publicly available data set formed by somatic mutations 
from 21 breast cancers are consistent with previous knowledge.
\\
...
%\href{rdrummond@gmail.com}{\texttt{rdrummond@cipe.accamargo.org.br}},
%\href{rvalieris@gmail.com}{\texttt{rvalieris@cipe.accamargo.org.br}},
...
% The use of these allow to identify molecular sub-types that permit
% the stratification of patients for subsequent targeted therapies.
