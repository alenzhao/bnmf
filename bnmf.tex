...
\abstract{%
 \textbf{Motivation:} All cancer types harbour somatic mutations,
ranging from hundreds to thousands of events. Beyond understanding the
prevalence and types of somatic mutations in cancer genomes, the
understanding of the causes and features of distinctive mutational
process that lead to neoplastic transformation is crucial to
comprehend cancer biology. Dietary compounds, tobacco consumption,
radiation exposure and viral infections are among the most common
environmental factors that lead to mutations. These acquired genomic
variations (somatic mutations) drive an evolutionary process where
some cell clones gain growth advantage and lead to tumour
development. The identification of mutation signatures and their
underlying mutational processes may be central to understanding of
...
\State Iterate the Gibbs sampler to obtain  $\big(Z^{(r)},
...
iterate the Gibbs sampler to obtain final sequence of samples
...
For the hierarchical model developed throughout, the Bayes
factor is given by
\begin{align*}
  \ell(M| \eta_N, \Mord)
  = 
   \int \Big\{ 
     & p(M, Z|\theta_N, \Mord) p(\theta_N|\psi_N, \Mord)   \\
     & p(\psi_N|\eta_N, \Mord)\Big\}\ dZ d\theta_N d\psi_N.
\end{align*}
This integral is not available in closed form but its value may be
estimated by using the output generated by the MCMC EM method
described here.